    ...
    { httpRequest: HttpRequest =>
      ...
      val (requestParams, newEntity) = httpRequest.entity match {
        case HttpEntity.NonEmpty(ContentType(MediaTypes.`application/x-www-form-urlencoded`, _), data) =>
          ...
          (pairs.toMap, HttpEntity(ContentType(MediaTypes.`application/x-www-form-urlencoded`), pairs map {t => s"${t._1}=${t._2}"} mkString "&"))
        case e => (Map(), e)
      }
      ...
      val newHeaders = List(RawHeader("Authorization", "OAuth " + oauth))
